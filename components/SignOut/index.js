...
import { View,Text,Dimensions,Image,TouchableOpacity,StatusBar,Button, Alert} from 'react-native';
...
function CustomDrawerContent(props,navigation) {
    ...
    function Atualizar(){
      ...
      .ref('/SOS/')
      .update({valor:false}),
      Alert.alert('Enviado com sucesso','seu SOS foi enviado com sucesso');      
    }  
    ...
    const [focus3,setFocus3]=useState(false)
     ...
     return (
      ...
      <View style={{flexDirection:'row',alignItems:'center',alignContent:'center',backgroundColor:'#a2c6f2',opacity:0.85,borderRadius:5}}>
        ...
        />
        <Text style={{fontSize:28,fontFamily:'notoserif'}} > HomeTech</Text>
        </View>
        <DrawerItem
        label="   Home"
      ...
      <DrawerItem
        label="Deslogar"
        icon={({color, size }) =><MaterialCommunityIcons name={'logout'} size={28} color={color} />}
        onPress={toggleModal}
        activeTintColor='#FCA700'
        inactiveTintColor='white'
      /> 
...
<DrawerItem
style={{marginTop:'35%',backgroundColor:'#E7E7E7'}}
        label="  S.O.S"
        icon={({ focused, color, size }) =><MaterialCommunityIcons name={'car-brake-alert'} size={40} color={'red'} focused={focused}/>}
        onPress={Atualizar}
        activeTintColor='#FCA700'
        inactiveTintColor='red'
      />
        <View style={{alignItems:'center',marginTop:'100%',marginBottom:300}}>
        </View>
